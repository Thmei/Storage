...
int polynomial::display(int *coeff, int degree) {
    int i, j;
    for (i = degree; i >= 0; i--) {  //using i-- to check descending degree, start with highest
        cout << coeff[i] << "x^" << i;
        if ((i - 1) != -1)
            cout << "+";
    }
...
void polynomial::addition(polynomial P1, polynomial P2) {
    ...
    if (P1.degree == P2.degree) { //if degrees are equal simply add coefficients
        for (i = P1.degree; i >= 0; i--)
            add[i] = P1.coeff[i] + P2.coeff[i];
    }

    if (P1.degree > P2.degree) { // if if degrees don't agree follow polynomial order of operations in calculations
